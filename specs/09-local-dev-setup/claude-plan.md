...
**Remove S3 credential fallback:** Remove the fallback that sets `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` to `SUPABASE_SECRET_KEY` when extraction fails. When `.env.local` is the source of truth, this fallback is unreachable and should not exist as silent behavior.
...
- S3 / Storage: `SMGR_S3_ENDPOINT`, `SMGR_S3_BUCKET`, `SMGR_S3_REGION`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_ENDPOINT_URL_S3`
...
**Checks to perform:**
1. Required env vars are non-empty: `SMGR_API_URL`, `SMGR_API_KEY`, `SUPABASE_SECRET_KEY`, `ENCRYPTION_KEY_CURRENT`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`
