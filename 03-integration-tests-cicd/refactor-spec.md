...
Feature: Multi-tenant data isolation
  ...
  Scenario: Search results are scoped to the requesting user
    When Alice calls search_events with her user_id
    Then results contain only Alice's events

    When Alice calls search_events with Bob's user_id
    Then results are empty (RLS blocks the underlying data)

  Scenario: Stats are scoped to the requesting user
    When Alice calls stats_by_content_type with her user_id
...
Feature: Media lifecycle from upload to search
  ...
  Scenario: Upload a photo and find it via search
    When the user uploads a JPEG to S3
    And the watcher detects the new object
    And an enrichment is created with description "sunset over mountains"
    Then searching for "sunset" returns the photo
    And searching for "cat" does not return the photo

  Scenario: Filter search results by content type and date range
    Given the user has photos and videos with enrichments
    When searching with content_type=photo
    Then only photos are returned
    When searching with a date range
    Then only events within that range are returned

  Scenario: Stats reflect the user's actual data
    Given the user has 2 photos and 1 video
    When the user requests stats_by_content_type
    Then the response shows photo=2, video=1

  Scenario: Enrichment status tracking
    Given the user has 3 events, 1 enriched
    When the user requests enrich status
    Then pending=2, enriched=1

  Scenario: Watched key upsert updates metadata on re-scan
    Given a watched key exists with etag "abc"
    When the same key is upserted with etag "def"
  ...
  Scenario: User A cannot see User B's media
    ...
    When User A queries events
    Then User B's events are not included
```

### Suite 4: Media Storage (`media-storage.test.ts`)

Keeps `media-s3.test.ts` mostly as-is (it's already clean), but with BDD naming.

```
Feature: S3-compatible storage operations
...
Extend the existing `setup.ts` with:

```typescript
/**
 * Creates a complete seed dataset for a test user.
 * Built against the CURRENT schema — no stale column references.
 */
export async function seedUserData(admin: SupabaseClient, userId: string, opts?: {
  eventCount?: number;
...
/**
 * Asserts an insert succeeded. Throws with full error context on failure.
 * Use this instead of raw .insert() in beforeAll to get clear failure messages.
 */
export async function assertInsert(
  description: string,
...
1. Starts Supabase (which applies migrations)
2. Extracts env vars from `supabase status`
3. Creates the media bucket
4. Runs `npm run test:integration` and `npm run test:media-integration`

**Changes needed:**
- Merge `test:integration` and `test:media-integration` into a single vitest config. Currently there are two separate configs (`vitest.integration.config.ts` and `vitest.media-integration.config.ts`) with different timeouts. Consolidate into one `vitest.integration.config.ts` that includes all files in `web/__tests__/integration/` with a 60s timeout.
- Remove the inline FTS smoke test from CI. It duplicates what `media-lifecycle.test.ts` will cover (search_events RPC), and inline psql in CI YAML is hard to maintain.
